...
2. **Figure classification (~7 ms/image on Apple Silicon, batched).** Reuses `docling-project/DocumentFigureClassifier-v2.5`, the EfficientNet-B0 classifier docling already ships and caches, so there is no extra dependency and no extra download. It labels each figure across 26 document categories, and a figure is dropped when the probability **summed over the decorative categories** reaches 0.80.

   Summing matters. The classifier splits its mass between `logo` and `icon` for the same picture, so thresholding the single top label kept obvious logos: the IBM DB2 wordmark scored `logo` 0.35 and `icon` 0.34, which a top-1 rule at 0.8 waves through even though the model is 69% sure the thing is decorative. The threshold is set from the 152 figures of a real 98-slide deck, every one inspected by eye: content misread as decorative (a headshot, two editorial illustrations, two 3D renders) tops out at 0.658, while vendor logos and pictograms run from 0.575 to 1.0, so 0.80 clears the content band by 0.14. The sum strictly contains the old rule, so switching could not un-drop anything: on that deck 51 classifier drops became 86, and all 35 additions were confirmed by eye to be pictograms, avatars or vendor logos (Oracle, Salesforce, Cloudera, Databricks, BigQuery, Cassandra, Presto, Flink, RabbitMQ, OpenAI).
